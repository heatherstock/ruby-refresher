...
def make_numbers_negative(number)
  return number * (-1) if number.positive? 
  return number 
end
...
def number_of_elements_that_are_palindromes(array)
  a = array.select { |word| word == word.reverse }
  a.size
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  (birthday.year..birthday.year + 15).each { |year|
...
def count_words_of_each_length_in_a_file(file_path)
  h = Hash.new
  f = File.read(file_path).to_s.gsub!(/[^0-9A-Za-z\s]/, '')
  f.each_line { |line|
    words = line.split(' ')
    words.each { |word|
      length = word.size
...
def fizzbuzz_without_modulo
  (1..100).each { |num| 
    if (num / 15.0) == (num / 15.0).round
      puts "fizzbuzz"
    elsif (num / 5.0) == (num / 5.0).round
      puts "buzz"
    elsif (num / 3.0) == (num / 3.0).round
      puts "fizz" 
    else puts num 
    end }
end
